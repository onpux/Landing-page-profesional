...
Cada requisito del proyecto se evalúa con cinco niveles de madurez, en orden creciente de certeza:

| Nivel | Pregunta | Significado | Ejemplo |
|-------|----------|-------------|---------|
| **Existe** | ¿El código está escrito? | El archivo contiene la implementación. | `index.html` tiene `<nav>`. |
| **Medido** | ¿Se ejecutó una prueba? | Hubo una acción de verificación (inspección manual, herramienta, test). | `axe DevTools` → contraste 4.5:1 verificado. |
| **Verificado** | ¿La prueba confirmó el requisito? | La medición tuvo un resultado positivo confirmado. | W3C Validator → "Document checking completed. No errors or warnings to show." |
| **Evidencia** | ¿Puedo demostrarlo objetivamente? | Existe un artefacto verificable (screenshot, reporte, log, hash). |
| **Rastreable** | ¿El cambio de hoy es igual al de ayer? | Hay un sistema que detecta alteraciones no intencionadas (git, snapshot checksum). |

Un requisito no está completo hasta que alcanza **Verificado**. La diferencia entre "implementado" y "verificado" es la diferencia entre "lo construí" y "lo probé y funciona".

---

## Regla de evidencia en comunicaciones

> **Ningún documento ni comunicación puede afirmar que un archivo fue creado, modificado o actualizado sin mostrar evidencia verificable (diff, contenido generado, confirmación de escritura o prueba de persistencia).**

Esto aplica a:

- Afirmaciones de la IA: "README creado" requiere mostrar el contenido o un diff.
- Afirmaciones en documentación: "Archivo X actualizado" requiere referenciar el cambio concreto.
- Estados en tablas: "✅ Implementado" requiere saber qué archivo contiene el código.

No es lo mismo decir "creé el archivo" que mostrar el archivo creado.
No es lo mismo decir "actualicé el roadmap" que mostrar el diff.

Esta regla extiende el modelo de verificación al plano de la comunicación.
Si no hay evidencia, no hay afirmación.
